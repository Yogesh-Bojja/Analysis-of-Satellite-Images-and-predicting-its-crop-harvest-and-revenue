...
+"Could not find context or error callback for event "+JSON.stringify(event),true);}
...
if(chatSpec.chatTheme.sv==GR34.MODERN_CI_VERSION){chatSpec.embeddedTheme.chatTheme.sv=chatSpec.chatTheme.sv;}}}catch(msg){log("MM12 Error: checkEmbeddedDivOnCI has error="+msg);obj=null;}
...
this.dataLoaded=true;this.init();this.load();if(!isWebSDK){this.preloadInqFramework();AL.preload();}
...
else if(!this.isV3Active()&&!this.isPersistentChatActive()){if(v3Lander.isBupMode&&inu1(this.getChat().getRule())){v3Lander.loadMbuDataForBU(this.getChat().getChatData().buID,function(ruleData){CHM.continueChatting(ruleData);});}
...
function fireCustomEvent(eventName,rule,evt,evtDataSupplementFcn){if(typeof evtDataSupplementFcn==='string'&&evtDataSupplementFcn.indexOf('function(')==-1){evtDataSupplementFcn="function() { return "+evtDataSupplementFcn+"}";var ruleLogData=rule?"Rule ID: "+rule.id:"rule: "+rule;let12("FlashPeer#fireCustomEvent get evtDataSupplementFcn as string. \n"+"evtDataSupplementFcn: "+evtDataSupplementFcn+
new Error().stack+ruleLogData,true);}
...
if(typeof dataFcn==='string'&&dataFcn.indexOf('function(')==-1){dataFcn="function() { return "+dataFcn+";}";let12("FlashPeer#fireCustomEvt get dataFcn as string. \n"+"dataFcn: "+dataFcn+
...
secureWithCustToken=isSecureWithCustToken;var secureWithCustTokenVar=new VER34("spcID",{},resources["session"]);secureWithCustTokenVar.z3(secureWithCustToken||false);if(!secureWithCustToken){var persistDataVar=new VER34("pcID",{},resources["session"]);var pcID=persistDataVar.z0();if(!objectEquals(pcID,persistCustID)){CONVM.reset();if(!CHM.getChat()||CHM.getChat().isAsyncChat()){renewCustomerId();}}
